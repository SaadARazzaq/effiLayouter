...
class PDFLineExtractor:
    ...
    def _translate_sentence_with_delay(self, sentence):
        ...
        if len(text) < 2 or text.isdigit() or all(not c.isalnum() for c in text):
            return text
        ...
        try:
            # Check character limit (Google Translate limit is ~5000 chars)
            if len(text) > 4500:
                logger.warning(f"Sentence too long ({len(text)} chars), truncating: {text[:100]}...")
                text = text[:4500]
